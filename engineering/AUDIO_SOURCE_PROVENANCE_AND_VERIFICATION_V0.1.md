...
| A-U396-4 (formal session) | Protocol cue track | `MIP_PROTOCOL_CUES_V1` when the selected protocol requires cues | MIP_OPERATIONAL_DEFINED | Active playbook cue architecture | Separate from the recipe component condition | Cues are generated from the committed protocol timeline, stored in the effective session audio configuration, and never added to `recipe.cues` | `src/main/main.js`, `public/audio-core.js` | protocol-cue separation test | Not a recipe/historical audio claim | Exact acoustic path remains outside PCM evidence |
...
| QUICK_CUSTOM | Centered pair and preview master gain | user supplied | USER_DEFINED / UNKNOWN_BLOCKED until saved | Audio Lab owner input | Exploratory only | Derived L=center−beat/2, R=center+beat/2; must be saved/versioned for formal use | `src/audio.js`, `public/app.js` | quick/custom PCM test | Not historical exact | Owner's intended use |
