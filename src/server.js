...
import { profiles, resolveProfile, validateProfile, assignOutcome, participantTarget, requestInstruction, CSPRNG, DeterministicRNG, analyzeStream, timingPlan, sha256 } from './engine.js';
...
function analysisFor(id, manifest) {
  ...
  return analyzeStream({ requested: manifest.actualObjectiveState, values, primary: [third, Math.floor(values.length * 2 / 3)], exploratory: [[0, third], [Math.floor(values.length * 2 / 3), values.length]] });
}
...
function runSession(id) {
  ...
  const { profile: p } = state; const rng = p.rng.provider === 'DETERMINISTIC_PRNG_TEST' ? new DeterministicRNG(state.seed) : new CSPRNG();
  ...
  const total = p.output.type === 'SINGLE_OUTCOME' ? 1 : (p.output.preBlocks + p.output.primaryBlocks + p.output.postBlocks) * (p.output.blockSize || 1); const values = [];
  for (let i = 0; i < total; i++) { const value = assignOutcome(p, rng); values.push(value); storage.appendOutput(id, { index: i, value, generatedUtc: new Date().toISOString(), block: i < total / 3 ? 'pre' : i < total * 2 / 3 ? 'primary' : 'post', rngProvider: rng.id, rngVersion: rng.version }); }
  ...
  state.values = values; state.analysis = analyzeStream({ requested: state.objective, values, primary: [Math.floor(total / 3), Math.floor(total * 2 / 3)], exploratory: [[0, Math.floor(total / 3)], [Math.floor(total * 2 / 3), total]] });
...
async function route(req, res, url) {
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/api/profiles') return send(res, 200, Object.values(profiles).map(p => ({ ...p, validation: validateProfile(p) })));
