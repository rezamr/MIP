...
import { PRESETS, quickRecipe, renderWav, validateRecipe, verifyArtifact } from '../src/audio.js';
...
test('layered phased-pink and Septon render deterministically', () => { const r={...PRESETS['A-U396-4'], id:'PHASED_TEST', mode:'PHASED_PINK_PATENT_5356368', noise:{algorithm:'PHASED_PINK_PATENT_5356368',seed:42,gain:.02}, septon:[{leftHz:200,rightHz:204,gain:.03}]}; const a=renderWav(r,.01), b=renderWav(r,.01); assert.equal(a.hashes.wav,b.hashes.wav); assert.equal(validateRecipe(r).valid,true); });
