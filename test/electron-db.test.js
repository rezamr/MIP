...
test("Electron SQLite authority migrates, chains evidence, and protects immutable rows", () => {
  ...
  assert.equal(
    db.db.prepare("SELECT MAX(version) v FROM schema_migrations").get().v,
    CURRENT_SCHEMA_VERSION,
  );
  assert.equal(db.profileList().length > 0, true);
  assert.equal(db.recipeList().length > 0, true);
  ...
  assert.throws(
    () =>
      db.db
        .prepare("DELETE FROM evidence_events WHERE session_id=?")
        .run(session.id),
    /immutable/,
  );
