...
MIP is a long-horizon forensic, technical, physiological, phenomenological, and information-transfer research program.

Its purpose is to determine, with the strongest evidence that can practically be obtained, what is actually occurring during the class of sessions historically associated with Gateway, Hemi-Sync, Monroe, Bentov, CENTER LANE, remote-viewing programs, related altered-state methods, and the project owner's own repeated session experiences.
...
Any of these outcomes is scientifically meaningful.

---

## End-state objective

MIP is not complete merely when an article is written, a historical protocol is reconstructed, or a compelling subjective session occurs.

The project remains open while material uncertainties remain that can reasonably be investigated.

The long-term objective is to reach the strongest defensible answer to all of the following:

1. **What state is being entered?**
   - phenomenologically;
   - physiologically;
   - neurologically where measurable;
   - acoustically in relation to the stimulus;
   - behaviorally.

2. **How is that state induced and controlled?**
   - audio architecture;
   - breathing;
   - attention;
   - posture;
...
3. **What is the historical operational system?**
   - public Gateway;
   - custom Monroe/Hemi-Sync work;
   - CENTER LANE / related government use;
   - missing/custom recordings;
   - biomonitoring;
...
MIP must not be prematurely closed merely because one search fails, one historical source is incomplete, one session produces no result, or one hypothesis is weakened.

For each important unresolved question, research should continue across all materially relevant domains until one of the following occurs:

- a parameter is recovered with adequate provenance;
- a hypothesis is strongly supported;
- a hypothesis is falsified or materially weakened;
- the search reaches documented saturation;
- access to the necessary evidence is presently impossible;
- or further investigation would add negligible information relative to cost/risk.

`UNKNOWN` is a valid temporary conclusion, not a reason to invent a value.

When new evidence changes the model, MIP must revise protocols and research priorities rather than defend an outdated design.

All revisions must be documented with the reason, evidence, and expected consequence.
...
However, the word **communication** in a session title is an operational label, not evidence that an external communicating agent or nonlocal substrate has been established.

Every Communication Session must preserve separately:
...
Durable MIP records may use these classes:
...
The existing historical session IDs `S0001`, `S0002`, etc. remain stable. Session class is metadata, not a replacement identifier.

---

## Research-advisor role

The assistant/research coordinator is expected to function as a rigorous project advisor, not as a cheerleader for any preferred theory.

Responsibilities include:

- maintaining the evidence hierarchy;
- identifying confounds and alternative explanations;
- designing stronger tests;
- preserving raw observations before interpretation;
- monitoring methodological and physical/psychological safety risks;
- identifying when a session should be terminated or a protocol modified;
- searching historical and modern technical literature;
- tracing missing archival material;
...
The advisor must not upgrade subjective certainty into objective fact without independent evidence.

The advisor must also not dismiss a repeated observation merely because its mechanism is currently unknown.

---

## Safety doctrine

Exploration must never depend on deliberately escalating loss of voluntary control, extreme physiological stress, sleep deprivation, dangerous breathing practices, unsafe sound levels, drugs, electrical stimulation, or other avoidable hazards.
...
The highest-value outcome is not a dramatic experience. It is a result that remains meaningful after controls.

A major claim requires progression through the MIP evidence ladder:

- `L0` single subjective observation;
- `L1` repeated subjective observation;
- `L2` association with a controlled condition;
...
A potential discovery of exceptional importance must be held to a correspondingly high evidence standard.
