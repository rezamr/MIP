...
// Configuration fingerprints remain part of the authoritative backend payload,
// but are omitted from owner-facing JSON disclosures because they are integrity
// identities rather than actionable recipe settings.
const ownerDisplayJson = (value) => {
  const omit = new Set(["configFingerprint", "configHash", "referenceConfigFingerprint"]);
  const strip = (node) => {
    if (Array.isArray(node)) return node.map(strip);
    if (!node || typeof node !== "object") return node;
    return Object.fromEntries(Object.entries(node).filter(([key]) => !omit.has(key)).map(([key, child]) => [key, strip(child)]));
  };
  return JSON.stringify(strip(jsonSafe(value)), null, 2);
};
